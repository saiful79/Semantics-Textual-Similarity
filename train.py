...
# python src/train.py --config_path configs/snli.sample.config
...
def output_probs(probs, label_vocab):
    ...
    for i in xrange(probs.size):
        outut = (label_vocab.getWord(i), probs[i])
        out_string.append(outut)
    # x = out_string.sort(key = operator.itemgetter(1), reverse = True)
    # print(x)
        ...
        with open("new_file.txt",'w') as file:
            if outpath is not None:
                ...
                for i in xrange(cur_batch.batch_size):
                    ...
                    "probs": str(output_probs(probs[i], label_vocab)),
                    }

                    x = colored("Sentence 1: ", 'green')+sentence1+colored("\nSentence 2: ", 'green')+sentence2+colored("\nPredictions : ", 'green')+label_vocab.getWord(predictions[i])
                    # print("===========================================================\n")
                    print("Output Result:\n")
                    print(x)
                    print(" ------------------------------------")
                    ...
                    print("|-------------------|----------------|")
                    cls_p= output_probs(probs[i], label_vocab)
                    for i in cls_p:
                        # predi_split = i.split(":")
                        if i[0]=="contradiction":
                            print("|",i[0],"    |   ",i[1],' |')
                        else:
